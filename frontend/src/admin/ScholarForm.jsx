...
const ScholarForm = ({ onSuccess }) => {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      ...
      if (compressedSize > 500 * 1024) {
        // If still larger than 500KB, compress more
      ...
      setErrors((prev) => ({
        ...prev,
        image: "Error processing image. Please try a different image.",
      }));
    }
  };
  ...
  const validateForm = () => {
    ...
    formData.works.forEach((work, index) => {
      if (!work.title.trim()) {
        newErrors[`work_title_${index}`] = `Work title ${
          index + 1
        } is required`;
      }
      if (!work.year) {
        newErrors[`work_year_${index}`] = `Publication year for work ${
          index + 1
        } is required`;
      }
    });
  ...
  const handleSubmit = async () => {
    ...
    try {
        ...
        JSON.stringify(
          {
            ...
            image: scholarData.image
              ? `[IMAGE_DATA_${scholarData.image.length}_CHARS]`
              : "NO_IMAGE",
          },
          null,
          2
      ...
      if (error.message.includes("You must be logged in")) {
        ...
        errorMessage =
          "Validation failed: Please check all fields and try again. Make sure you are logged in as an admin.";
        ...
        errorMessage =
          "Image is too large. Try removing the image or uploading a smaller one.";
      } else if (error.message.includes("timeout")) {
        errorMessage =
          "Request timeout. The image might be too large. Try without image.";
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-bold mb-2">
                Add New Islamic Scholar
              </h2>
              <p className="text-emerald-100">
                Register a new scholar in the Islamic library
              </p>
            </div>
